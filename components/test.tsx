...
const TimelineWithSpiral = () => {
  ...
  const timelineItems = [
    { title: "Registration Opens", date: "January 15, 2025", left: true,icon:<NotebookPen className="w-6 h-6 text-gray-300 mb-2 "/> },
    { title: "Opening Ceremony", date: "March 1, 2025", left: false,icon:<Newspaper className="w-6 h-6 text-gray-300 mb-2 "/> },
    { title: "Hacking Begins", date: "March 2, 2025", left: true,icon:<Megaphone className="w-6 h-6 text-gray-300 mb-2 "/> },
    { title: "Final Presentation", date: "March 4, 2025", left: false,icon:<Flame className="w-6 h-6 text-gray-300 mb-2 "/> },
  ];

  return (
    <div className=" mx-auto relative bg-gray-50">
      <h2 className="text-4xl font-bold text-center mb-4 bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent font-Playfair-Display italic">
        Event Timeline
      </h2>
      <div>
        <div className="h-auto w-screen mx-auto flex flex-col justify-center items-center relative">
          {/* This is your canvas layer */}
          ...
          {timelineItems.map((item, index) => (
            <div key={index} className=" mt-6 mb-6 w-2/3 z-20">
              {item.left ? (
                <div className="w-full flex justify-start ">
                  <div className="min-w-96 p-8 border-2 border-gray-200 rounded-3xl hover:bg-gray-100 duration-200 bg-white relative">
                    <div className="w-6 h-6 rounded-full border-2 border-gray-200 absolute top-1/2 -translate-y-1/2 -right-3 bg-white"></div>
                    {item.icon}
                    <h3 className="font-bold text-xl font-Playfair-Display italic">{item.title}</h3>
                ...
                <div className="w-full flex  justify-end">
                  <div className="min-w-96 p-8 border-2 border-gray-200 rounded-3xl hover:bg-gray-100 duration-200 bg-white relative">
                    <div className="w-6 h-6 rounded-full border-2 border-gray-200 absolute top-1/2 -translate-y-1/2 -left-3 bg-white"></div>
                    {item.icon}
                    <h3 className="font-bold text-xl font-Playfair-Display italic">{item.title}</h3>
                    <p className="text-gray-600">Lorem ipsum dolor sit amet </p>
                    <p className="text-gray-600">{item.date}</p>
